<template>
    <div v-for="quiz in quizData" :key="quiz.pk" class="quiz-inner">
      ...
      <p>정답 입력</p>
      <input v-model="answers[quiz.pk]" :id="quiz.pk" type="text" @keyup.enter="confirmSubmit(quiz)">
    </div>
</template>
...
const confirmSubmit = (quiz) => {
  const answer = answers.value[quiz.pk]
  const confirmation = window.confirm(`${answer}을/를 답안으로 제출합니다. 확실합니까?`)
  if (confirmation) {
    submitAnswer(quiz)
  } else {
    answers.value[quiz.pk] = ''
  }
}
</script>
